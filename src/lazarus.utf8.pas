...
// Get actual length of text on terminal based on character width
function UTF8TerminalLength(S: String): LongInt;
...
begin
  ...
  begin
    CS := UTF8Decode(UTF8Copy(S, I, 1));
    C := Word(CS[1]);
    if ((C >= $20) and (C <= $1FFF)) or ((C >= $FF61) and (C <= $FF9F)) then
      Inc(Result)
    else
    if ((C >= $2000) and (C <= $FF60)) or (C >= $FFA0) then
      Inc(Result, 2);
  end;
end;
...
begin
  Result := '';
  if (N <= 0) or (UTF8TerminalLength(AUtf8Char) <> 1) then Exit;
...
begin
  Result := S;
  if UTF8TerminalLength(AUtf8Char) <> 1 then Exit;
  L := UTF8TerminalLength(Result);
  if L < N then
...
var
  ULen: PtrInt;
begin
  ULen := UTF8TerminalLength(S);
  if ULen < N then
    begin
      Result := Utf8StringOfChar(AUtf8Char,(N div 2) - (ULen div 2)) + S;
      Result := Result + Utf8StringOfChar(AUtf8Char, N - UTF8TerminalLength(Result));
    end
  else
    Result := S;
end;
